 ...
 *
 *	purpose: Implementing UDP on top of IPv4
 *
 ************************************************************************************/
...
public class UdpClient {
    public static void main(String[] args) throws IOException {
        try (Socket socket = new Socket("18.221.102.182", 38005)){
            ...
            byte[] message = {(byte) 0xDE,
                    (byte) 0xAD,
                    (byte) 0xBE,
                    (byte) 0xEF};
            os.write(Ipv4(4, message));
            System.out.print("Handshake response: ");
            ResponseCode(is);
            System.out.println();
            byte[] portNum = new byte[2];
            portNum[0] = (byte) is.read();
            portNum[1] = (byte) is.read();
            int port = ((portNum[0] & 0xFF) << 8) | (portNum[1] & 0xFF);
            ...
            for (int i = 0; i < 12; ++i){
                ...
                System.out.println("Sending packet with " +
                        size +
                        " bytes of data");
    ...
    public static void ResponseCode(InputStream is) throws IOException{
        System.out.print("0x");
        byte fromServer;
        for (int i = 0; i < 4; ++i){
    ...
    public static byte[] Ipv4(int size, byte[] data){
